...
def combine_results(output_dir, L, p_proj):
...
def main():
    ...
    # Fixed parameters
    p_ctrl_values = np.linspace(args.p_c - args.delta_p, args.p_c + args.delta_p, args.num_p_ctrl)

    # Create all parameter combinations including chunk indices
    all_params = [(args.L, args.p_proj, chunk_idx) 
    ...
    if rank == 0:
        final_results = combine_results(output_dir, args.L, args.p_proj)
